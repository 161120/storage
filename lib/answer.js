...
module.exports = class Answer extends Model {
	...
	static normalizeCreate(data) {
		data = _.pick(data, 'questionId', 'title', 'status', 'userId', 'type', 'description');
	...
	static normalizeUpdate(data) {
		data = _.pick(data, 'id', 'title', 'status', 'countVotes', 'type', 'description');
	...
	static createSlugKey(data) {
		return utils.md5([data.questionId.trim(), data.slug.trim()].join('|'));
	}
